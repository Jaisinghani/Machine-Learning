    ...
    for (train, test) in (k_test_fold.split(normalized_glass_features, glass_type)):
        ...
        for cvalue in cvalues:
            for gammavalue in gammavalues:
                classifier = SVC(kernel="rbf", C=cvalue, gamma =gammavalue, decision_function_shape="ovo")
                classifier.fit(training_features,training_glassType)
                validation_true, validation_pred = validation_glassType, classifier.predict(validation_features)
                accuracy_Validationset = metrics.accuracy_score(validation_true, validation_pred)
        ...
        #Training a new model on the entire 4 folds with optimal Hyper Parameters
        classifier_1 = SVC(kernel="rbf", C=validation_acuracies[-1][0], gamma=validation_acuracies[-1][1], decision_function_shape="ovo")
        classifier_1.fit(training_dataset_features,training_dataset_types)
        ...
        accuracy_test = metrics.accuracy_score(test_true, test_pred)
        print("accuracy_test  :", accuracy_test)
        test_accuracy_with_params.append((classifier_1.get_params().get('C'),classifier_1.get_params().get('gamma'), accuracy_test))
        test_accuracy.append(accuracy_test)
    print("Test Accuracies for all fold with params :", test_accuracy_with_params)
    print("Test Accuracies for all fold:", test_accuracy)
    average_accuracy = sum(test_accuracy) / len(test_accuracy)
...
def polynomialKernel(parameters):
    ...
    for (train, test) in (k_test_fold.split(normalized_glass_features, glass_type)):
            ...
            for gammavalue in gammavalues:
                for degreevalue in degreevalues:
                    for coefvalue in coefvalues:
                        classifier = SVC(kernel="poly", C=cvalue, gamma =gammavalue, degree=degreevalue, coef0= coefvalue, decision_function_shape="ovo")
                        classifier.fit(training_features,training_glassType)
                        validation_true, validation_pred = validation_glassType, classifier.predict(validation_features)
                        accuracy_Validationset = metrics.accuracy_score(validation_true, validation_pred)
        ...
        #Training a new model on the entire 4 folds with optimal Hyper Parameters
        classifier_1 = SVC(kernel="poly", C=validation_acuracies[-1][0], gamma=validation_acuracies[-1][1], degree=validation_acuracies[-1][2], coef0=validation_acuracies[-1][3], decision_function_shape="ovo")
        classifier_1.fit(training_dataset_features,training_dataset_types)

        end_time_training = time.clock()
        print("Time taken to train for one fold for poly kernel is :", (end_time_training- start_time_training))
    ...
    print("Test Accuracies for all fold:", test_accuracy)
    average_accuracy = sum(test_accuracy) / len(test_accuracy)
    print("average accuracy for poly SVM is :", average_accuracy)
...
def sigmoidKernel(parameters):
    ...
    for (train, test) in (k_test_fold.split(normalized_glass_features, glass_type)):
        ...
        for cvalue in cvalues:
            for gammavalue in gammavalues:
                for coefvalue in coefvalues:
                    classifier = SVC(kernel="sigmoid", C=cvalue, gamma =gammavalue, coef0= coefvalue, decision_function_shape="ovo")
                    classifier.fit(training_features,training_glassType)
                    validation_true, validation_pred = validation_glassType, classifier.predict(validation_features)
                    accuracy_Validationset = metrics.accuracy_score(validation_true, validation_pred)
        ...
        #Training a new model on the entire 4 folds with optimal Hyper Parameters
        classifier_1 = SVC(kernel="sigmoid", C=validation_acuracies[-1][0], gamma=validation_acuracies[-1][1], coef0=validation_acuracies[-1][2], decision_function_shape="ovo")
        classifier_1.fit(training_dataset_features,training_dataset_types)
